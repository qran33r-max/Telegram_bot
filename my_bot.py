...
facts = {
    ...
    "🏛️ تاريخ": [
        ...
        "👑 الملك فيصل بن عبدالعزيز حكم السعودية",
        "🕌 الدولة العباسية كانت مركزاً للعلم"
    ],
    "🐧 حيوانات": [
        ...
        "🐫 الجمل سفينة الصحراء",
        "🐬 الدلفين من أذكى الحيوانات"
    ],
    "⚽ رياضة": [
        "⚽ كرة القدم هي الرياضة الأكثر شعبية في العالم",
        "🏆 كأس العالم أقيم أول مرة عام 1930",
        "🐐 لقب GOAT في كرة القدم يعني أعظم لاعب على مر التاريخ",
        "🇦🇷 ميسي فاز بكأس العالم 2022 مع الأرجنتين",
        "🇵🇹 كريستيانو رونالدو هو الهداف التاريخي لكرة القدم",
        "🏅 الألعاب الأولمبية تقام كل 4 سنوات",
        "🎾 رافاييل نادال يلقب بملك الملاعب الترابية",
...
def show_stats(chat_id):
    ...
    if not stats_counter:
        ...
        for i, (cat, count) in enumerate(sorted_stats[:5], 1):
        ...
        for update in updates["result"]:
            if last_update_id is None or update["update_id"] > last_update_id:
                ...
                if "message" in update:
                    ...
                    try:
                        if text == "/start":
                            welcome = "✨ مرحباً بك في بوت المعلومات المتطور!\n\nاختر قسم من الأزرار 👇\n\n⚽ رياضة: معلومات رياضية جديدة\n🎲 عشوائي: معلومة من أي قسم\n📊 إحصائيات: الأقسام الأكثر طلباً"
